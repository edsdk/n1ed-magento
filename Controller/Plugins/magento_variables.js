        ...
        tinymce.create('tinymce.plugins.magentovariable', {
            ...
            init: function (editor, url) {
                ...
                editor.addCommand('openVariablesSlideout', function (commandConfig) {
                    ...
                    MagentovariablePlugin.loadChooser(
                        window.mveditor.settings.mvOpts.url,
                        window.mveditor.settings.wysiwyg.getId(),
                        selectedElement
                    );
                });

                editor.ui.registry.addIcon('magentovariable', '<img src="/upload/plugins/mvpic" />');
            ...
            encodeVariables: function (content) {
                content = content.gsub(/\{\{config path=\"([^\"]+)\"\}\}/i, function (match) {
                    ...
                    magentoVariables = JSON.parse(window.mveditor.settings.mvOpts.placeholders);
            ...
            decodeVariables: function (content) {
                ...
                [].forEach.call(doc.querySelectorAll('span.magento-variable'), function (el) {
                    ...
                    if ($el.hasClass('magento-custom-var')) {
                        $el.replaceWith(
                            windows.mv.customDirectiveGenerator.processConfig(
                                Base64.idDecode(
                                    $el.attr('id')
                                )
                            )
                        ...
                        $el.replaceWith(
                            window.mv.configDirectiveGenerator.processConfig(
                                Base64.idDecode(
                                    $el.attr('id')
                                )
                            )
